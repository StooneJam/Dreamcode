...
Bytedance project · 业务场景级 vibecoding · 2026-05-20 ~ 2026-06-05 (17 天迭代)

## 项目目标

模拟数字调研小组，自动完成 **公开信息采集 → 结构化竞品报告输出** 的全链路工作流。
首发垂直：**办公软件**（飞书 / 钉钉 / 腾讯会议 / Zoom），架构支持 config 切换其他赛道。
...
短期 checkpointer 直接用 `langgraph.checkpoint.sqlite.SqliteSaver`，不另写包装。
...
- `docs/architecture.html` — LangGraph 架构图（浏览器打开）
- `docs/decisions.md` — 决策日志（ADR，待写）
