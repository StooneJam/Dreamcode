...
编号从 [1] 开始，按产品顺序排列（目标产品 → 竞品 A → 竞品 B），同一来源不重复。请务必保证每条数据来源之间换行。
...
- **正文不插入角标编号**：不在句末写 `[N]`，来源统一在报告末尾"数据来源"节集中列出。请务必保证每条数据来源之间换行。
...
- 不要在正文句末插入 `[N]` 角标，来源统一在末尾数据来源节列出;请务必保证每条数据来源之间换行。
