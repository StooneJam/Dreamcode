...
- `web_search(query, max_results)`：自然语言搜索，返回 `[{title, url, snippets}]`，snippets 是按 query 蒸出的逐字片段。**优先用此工具发现链接**
...
- **fetch_url 失败时**：换其他 URL 或仅靠 web_search 片段继续，**不要卡死**。**rationale 里必须说明哪些 URL 失败 + 你换了什么方向**
