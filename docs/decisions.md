...
## D-033 · Collector Phase 2 设计：每产品独立 ReAct + Send fanout 并发演示
**Status**: Accepted · **Date**: 2026-05-26
**Used by**: `cca/agents/collector.py::collect_one_product` / `collect_node` / `collect_dispatch_node` (TODO)

**Context**：Phase 1（`exploration_node`）跑通后规划 Phase 2 `collect_node`。Phase 2 的工作是对 PM 下发的每个 `CollectTask` 抓官网/定价/功能页，填实 `state.profiles[name]` 的 dimensions / pricing / sources。讨论两个核心设计点：(1) **编排方式**——单 ReAct loop 跑所有产品 vs 每产品独立 ReAct；(2) **工具集**——加哪些专用工具、约束哪些行为。

**Decision**：
- **B 编排：每产品独立 ReAct loop**（与 Insight 的 A 模式不同）
- **4 个工具**：`web_search` + `fetch_url` 复用 phase 1；新增 `finalize_profile` + `request_product_replacement`
- **`max_fetches = 5` per product**（prompt 层软约束）
- **Evidence 强约束**：`Fact.evidence.snippet` 必须是 `fetch_url` 返回 text 的原样片段，禁止凭训练知识编造

**Why B 编排（vs Insight 用的 A）**：
- **demo 视觉效果**：每产品独立 invoke 留下 LangGraph Send fanout 接入位，答辩演示三产品**并发抓取** 比顺序 1-2-3 视觉冲击强
- **失败隔离**：单产品 ReAct 异常（finalize 没调 / context 超长）只影响该产品，不连坐
- **每产品工作量大**：5 个 URL + 多维度抽取，单 loop 跑 3 产品 ≈ 30-50 turns；分开后每 loop ≤ 15 turns，DeepSeek context 压力小
- **代价**：失去跨产品共享上下文（无法"刚抓了钉钉的定价表，对比飞书"），但 Collector 本就不做横向对比（D-020 半步原则——Analyst 才横向对比）

**三层架构**：
- `collect_one_product(task, context) → dict`：单产品 ReAct，可被任何调用方使用（测试 / 顺序迭代 / Send target）
- `collect_node(state) → dict`：当前同步顺序版，逐个调 `collect_one_product` 后 merge。**用于 demo 脚本 + graph 接入前的 smoke test**
- `collect_dispatch_node(state)`：**占位 TODO**，graph 集成时换成 `return [Send("collect_one_product", payload) for task in ...]`，演示真并发

**为什么不立即写 dispatch_node**：graph 拼装是更大话题（路由 / 条件边 / 中断处理），单独立项。Phase 2 单跑通先。

**工具设计**：

| 工具 | 路径 | 触发场景 |
|---|---|---|
| `finalize_profile(product_name, profile_json)` | 正常路径 | 每个产品 ReAct **必须**调用一次提交结构化 ProductProfile |
| `request_product_replacement(product_name, reason, evidence)` | 异常路径 | 产品不存在 / 官网 404 / 数据完全采不到 → kind=data_gap，target=task_plan，PM 走 reroute 清空 task_plan 重派 |

**为什么不复用 phase 1 的 `challenge_pm`**：
- `challenge_pm` 的 target 硬编码 `initial_brief`（挑战 PM 阶段 1 的产品选择）
- Phase 2 阶段产品列表已锁定，要挑战的是 task_plan 这个具体产品采集任务的可行性
- 语义不同分两个工具更干净；也避免泛化 challenge_pm 接口

**`max_fetches = 5` 的选择**：
- 5 个页面足够覆盖：官网首页 + 定价页 + 1-2 个 priority_dimension 功能页 + 1 个备用
- 软约束（prompt 层）而非硬限制：LLM 通常遵守 token 预算约束；硬限制需要 wrapper 跟踪 per-product 计数，第一版不值得
- 升级触发：若实际跑出来 LLM 经常逼近 5 次仍信息不够 → 改为 7-8 或加 hard limit

**Evidence 反幻觉约束（D-016 证据驱动延续）**：
prompt 显式规定 "snippet 必须是 fetch_url 返回 text 的原样片段"。LLM 跑 ReAct 时倾向用搜索摘要凑数 evidence；强约束防止。检测靠下游（Insight / Analyst / call_report_reviewer）随机抽检 snippet 是否能在 sources 里的 URL 反查到。

**与 D-031 producer-owns 的关系**：Collector 写 `state.profiles[name]` 字段（dimensions/pricing/sources/website/product_type/target_users），不碰 Insight 的 sentiment 或 Analyst 的 swot。`state.profiles` 已有 `_merge_profiles` reducer（D-031），同 key 时按字段 merge，并发安全。

**Trade-offs**：
- B 编排比 A 多一层抽象（per-product context 切片），代码量 +30%
- 单产品独立 loop 失去跨产品视角；但 Collector 职责本就是采集而非分析，无损
- `max_fetches=5` 是经验值；定价复杂或多语言官网的产品可能不够

**升级触发**：
- 普遍单产品 5 次 fetch 不够 → 调高 / 改硬限制 + 单独 audit
- 真接 LangGraph Send 后并发演示效果不达预期 → 加 streaming 进度反馈（与 demo 流式输出 TODO 合并做）
